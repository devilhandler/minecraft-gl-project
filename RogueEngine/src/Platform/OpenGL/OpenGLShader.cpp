...
namespace Minecraft
{
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		if (in)
		{
			...
			result.resize(in.tellg());					// Resizing the string's size so it can fit the whole file binary
			in.seekg(0, std::ios::beg);					// Put the pointer to the start once more
			in.read(&result[0], result.size());			// And start reading from the first character until the end
			in.close();									// Then close the ifstream
		}
		else
		{
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string> shaderSources)
	{
		...
		// Now time to set the program into the m_RendererID.
		// This is to prevent an unsuccessful program to be made.
	...
	{
		GLint valueLocation{ glGetUniformLocation(m_RendererID, name.c_str()) };
		glUniform4f(valueLocation, values.x, values.y, values.z, values.w);
	}
